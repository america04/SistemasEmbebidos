...
try:
    # Ajustado al puerto que funcionó en tus pruebas
...
while cap.isOpened():
    ...
    # Dentro del bloque de la tecla R en detector_fatiga.py
    if key == ord('r') or key == ord('R'):
        print("Enviando comando de REINICIO...")
        inicio_ojos_cerrados = 0
        bostezo_en_progreso = False
        if arduino_conectado:
            try:
                arduino.reset_input_buffer() # Limpia datos viejos
                arduino.write(b'R')
                arduino.flush() # Fuerza el envío inmediato
